...
remove_permission_from_plist() {
    ...
    if plutil -extract "$permission_key" raw "$plist_file" >/dev/null 2>&1; then
        plutil -remove "$permission_key" "$plist_file"
        log_success "Removed permission: $permission_key"
    else
...
inject_permissions() {
    ...
    if [ "${IS_DOMAIN_URL:-false}" = "true" ]; then
        ...
        # Add network security settings - check if NSAppTransportSecurity exists first
        if ! plutil -extract "NSAppTransportSecurity" raw "$plist_file" >/dev/null 2>&1; then
            plutil -insert "NSAppTransportSecurity" -dict "$plist_file" 2>/dev/null || true
        fi
        # Try to add the arbitrary loads setting, but don't fail if it already exists
        plutil -insert "NSAppTransportSecurity.NSAllowsArbitraryLoads" -bool true "$plist_file" 2>/dev/null || \
        plutil -replace "NSAppTransportSecurity.NSAllowsArbitraryLoads" -bool true "$plist_file" 2>/dev/null || \
        log_info "NSAllowsArbitraryLoads setting already exists or could not be modified"
        permissions_added=$((permissions_added + 1))
    fi
...
create_permission_summary() {
    local summary_file="${OUTPUT_DIR:-build/ios/output}/PERMISSIONS_SUMMARY.txt"
    
    # Create directory if it doesn't exist
    local summary_dir=$(dirname "$summary_file")
    mkdir -p "$summary_dir"
    
    log_info "Creating permissions summary in: $summary_file"
...
main() {
    ...
    # Create permission summary (don't fail if this doesn't work)
    if create_permission_summary; then
        log_success "Permission summary created successfully"
    else
        log_warn "Permission summary creation failed, but continuing..."
    fi
    
    log_success "Dynamic permission injection completed successfully!"
    log_info "Permissions have been injected into Info.plist"
    log_info "Summary file created: ${OUTPUT_DIR:-build/ios/output}/PERMISSIONS_SUMMARY.txt"
